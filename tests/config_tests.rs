...
fn test_missing_required_env_vars() {
    ...
    unsafe {
        ...
        env::remove_var("LLM_CHAT_ENDPOINT");
...
fn test_default_port() {
    // Set required vars with minimal valid values
    set_minimal_config();
    unsafe {
        env::remove_var("PORT");
    }
...
fn test_invalid_port() {
    set_minimal_config();
    unsafe {
        env::set_var("PORT", "99999"); // Invalid port number
    }
...
fn test_streaming_mode_parsing() {
    ...
    for (mode_str, expected_mode) in modes {
        unsafe {
            env::set_var("STREAMING_MODE", mode_str);
        }
        let config = Config::from_env().expect("Should load config");
        assert_eq!(
...
fn test_retry_config() {
    ...
    unsafe {
        env::set_var("ENABLE_RETRIES", "false");
...
/// Helper function to set minimal valid configuration for tests
fn set_minimal_config() {
    ...
    unsafe {
        env::set_var("GCP_SERVICE_ACCOUNT_KEY", key_b64);
        env::set_var("LLM_URL", "https://test.example.com/v1/");
        env::set_var("LLM_CHAT_ENDPOINT", "test-model:streamRawPredict");
        env::set_var("LLM_MODEL", "test-model");
    }
}
